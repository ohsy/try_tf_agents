...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--epsilon_decay_period', type=float, help="period over which to decay epsilon from 1.0 to epsilon_greedy")
    ...
    epsilon_decay_period = config['epsilon_decay_period'] if args.epsilon_decay_period is None else args.epsilon_decay_period
    # period over which to decay epsilon, from 1.0 to epsilon_greedy
    ...
    # game is defined here for epsilon_decay to use game.train_step
    if 'multiagent' in agentName:
    ...
    epsilon = tf.compat.v1.train.polynomial_decay(
            ...
            global_step=game.train_step, 
            decay_steps=epsilon_decay_period, 
            end_learning_rate=epsilon_greedy)

    if 'multiagent' in agentName:
        agents = get_agents(config, tf_observation_spec, tf_action_spec, epsilon)  # epsilon_greedy)  # list of agents 
        tf_agent_collect_data_spec = get_tf_agent_specs_for_multiagent(agents, tf_action_spec)
    else: 
        agent = get_agent(config, tf_observation_spec, tf_action_spec, epsilon)  # epsilon_greedy)  # one agent 
