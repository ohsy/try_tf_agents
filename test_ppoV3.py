...
# HalfCheetah results in the problem of "no module of mujoco_py" 
flags.DEFINE_string('env_name', 'Pendulum-v1', 'Name of an environment')
flags.DEFINE_integer(
    'replay_buffer_capacity', 1001, 'Replay buffer capacity per env.'
)
flags.DEFINE_integer(
    'num_parallel_environments', 30, 'Number of environments to run in parallel'
)
flags.DEFINE_integer(
...
def train_eval(
    ...
    # env_load_fn=suite_gym.load,  # to solve the problem of "no module of mujoco_py" 
    random_seed=None,
  ...
  with tf.compat.v2.summary.record_if(
    ...
    saved_model = policy_saver.PolicySaver(tf_agent.policy, train_step=global_step)
    ...
    while environment_steps_metric.result() < num_environment_steps:
      ...
      if global_step_val % eval_interval == 0:
        ...
        avg_return = compute_avg_return(eval_tf_env, tf_agent.policy, num_eval_episodes)
        logging.info('step = %d: Average Return = %.3f', global_step_val, avg_return)
        print('step = {0}: Average Return = {1}'.format(global_step_val, avg_return))
      ...
      if global_step_val % log_interval == 0:
        ...
        print('step = {0}, loss = {1}'.format(global_step_val, total_loss))
